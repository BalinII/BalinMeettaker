...
export const useCompletion = () => {
  ...
  const submit = useCallback(
    async (speechText?: string) => {
      ...
      try {
        ...
        const useMinuteSmithAPI = await shouldUseMinuteSmithAPI();
        // Check if AI provider is configured
        if (!selectedAIProvider.provider && !useMinuteSmithAPI) {
          setState((prev) => ({
            ...prev,
        ...
        try {
          ...
          for await (const chunk of fetchAIResponse({
            provider: useMinuteSmithAPI ? undefined : provider,
  ...
  useEffect(() => {
    ...
    const handleStorageChange = async (e: StorageEvent) => {
      if (e.key === "minutesmith-conversation-selected" && e.newValue) {
        try {
        ...
        if (!hasPermission) {
          ...
          if (!hasPermissionNow) {
            setState((prev) => ({
              ...prev,
              error:
                "Screen Recording permission required. Please enable it by going to System Settings > Privacy & Security > Screen & System Audio Recording. If you don't see MinuteSmith in the list, click the '+' button to add it. If it's already listed, make sure it's enabled. Then restart the app.",
            }));
