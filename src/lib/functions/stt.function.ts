...
import { shouldUseMinuteSmithAPI } from "./local-api";

// MinuteSmith STT function
async function fetchMinuteSmithSTT(audio: File | Blob): Promise<string> {
  try {
    ...
    return `MinuteSmith STT Error: ${errorMessage}`;
  }
}
...
export async function fetchSTT(params: STTParams): Promise<string> {
  ...
  try {
    ...
    // Check if we should use MinuteSmith API instead
    const useMinuteSmithAPI = await shouldUseMinuteSmithAPI();
    if (useMinuteSmithAPI) {
      return await fetchMinuteSmithSTT(audio);
    }
